...
语音模式不会申请摄像头权限、不会创建视频轨道。视频模式为 640×480、目标约 15 FPS；采集画面按比例居中裁切后编码 JPEG，不拉伸。接收端解码繁忙时只保留最新 JPEG，WebSocket backlog 超过 256 KiB 时跳过新编码或发送，避免延迟持续累积。
...
5. 观察 LongPet 远端视频满屏裁切、本地右上角小窗；触屏显示挂断，4 秒后隐藏。
